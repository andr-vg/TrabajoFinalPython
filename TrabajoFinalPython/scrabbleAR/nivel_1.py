...
def abrir_arch_tablero():
    ...
    if "win" in sys.platform:
        arch = open(absolute_path + '\\Datos\\info\\tablero.csv',
                    "r")  # esto lo agregue porque no me encontraba el archivo
    else:
...
def crear_layout():
    ...
    fila_fichas = [
        sg.Button(button_text=letras_valores[i], key=letras_keys[i], size=(4, 2), button_color=('white', 'blue')) for i
        in range(7)]
    fila_botones = [sg.Button("Confirmar", key="-c"), sg.Button("Deshacer", key="-d"), sg.Button("Terminar", key="-t"),
                    sg.Button("Posponer", key="-p"),
